...
func setDelAAChange(anno TransAnno, trans refgene.Transcript, cstart int, cend int) TransAnno {
	cdna := trans.CDNA()
...
func AnnoDel(snv io.Variant, trans refgene.Transcript) TransAnno {
	...
	if snv.Start < trans.CdsStart && snv.End > trans.CdsEnd {
		...
		if region1.Type == refgene.RType_CDS {
			...
			anno = setDelAAChange(anno, trans, cStart, cLen)
		} else {
			// intron
			// ...+++,,,...
			//        |--|
			...
			anno = setDelAAChange(anno, trans, cStart+1, cLen)
		}
		...
		if region1.Equal(region2) {
			...
			if region1.Type == refgene.RType_CDS {
				...
				anno = setDelAAChange(anno, trans, cStart+1, cEnd)
			}
		}
	}
	return anno
