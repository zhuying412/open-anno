...
func setSubAAChange(anno TransAnno, trans refgene.Transcript, cstart int, cend int, alt string) TransAnno {
	cdna := trans.CDNA()
	...
	if (len(cdna)-len(ncdna))%3 == 0 {
		...
		if len(aa2) == 0 {
			...
			if len(aa1) == 1 {
				anno.AAChange = fmt.Sprintf("p.%s%ddelins%s", seq.AAName(aa1, AA_SHORT), start, seq.AAName(aa2, AA_SHORT))
			} else {
				anno.AAChange = fmt.Sprintf(
					"p.%s%d_%s%ddelins%s",
					seq.AAName(aa1[0], AA_SHORT),
...
func AnnoSub(snv io.Variant, trans refgene.Transcript) TransAnno {
	...
	anno := NewTransAnno(trans, region1, region2)
	if isExonSplicing {
	...
	if snv.Start < trans.CdsStart && snv.End > trans.CdsEnd {
		...
		if region1.Type == refgene.RType_CDS {
			...
			anno = setSubAAChange(anno, trans, cStart, cLen, snv.Alt)
		} else {
			// intron
			// ...+++,,,...
			//        |--|
			...
			anno = setSubAAChange(anno, trans, cStart+1, cLen, snv.Alt)
		}
		...
		if region1.Equal(region2) {
			...
			if region1.Type == refgene.RType_CDS {
				...
				anno = setSubAAChange(anno, trans, cStart+1, cEnd, snv.Alt)
			}
		}
	}
	return anno
