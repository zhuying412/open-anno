...
func RunAnnotate(variants variant.IVariants, databaseFile string) map[string]FilterBasedAnno {
	...
	annoMap := make(map[string]FilterBasedAnno)
	var headers []string
	var _variant variant.Variant
	var _anno FilterBasedAnno
	for i := 0; i < variants.Len(); {
		...
		if len(_anno) == 0 {
			if line, isEof := ReadLine(reader); isEof {
				...
				_variant, _anno = ReadFilterBasedFields(headers, strings.Split(line, "\t"))
			}
		}
